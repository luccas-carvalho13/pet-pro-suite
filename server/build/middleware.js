...
export async function requireAuth(req, res, next) {
    try {
        ...
        const { rows } = await pool.query(`SELECT company_id FROM public.profiles WHERE id = $1`, [req.userId]);
        req.companyId = rows[0]?.company_id ?? null;
        const superadmin = await pool.query(`SELECT 1 FROM public.user_roles WHERE user_id = $1 AND role = 'superadmin' LIMIT 1`, [req.userId]);
        req.isSuperAdmin = superadmin.rows.length > 0;
        let adminRoleRows = 0;
        if (req.companyId) {
            const adminRole = await pool.query(`SELECT 1 FROM public.user_roles WHERE user_id = $1 AND company_id = $2 AND role IN ('admin', 'superadmin') LIMIT 1`, [req.userId, req.companyId]);
