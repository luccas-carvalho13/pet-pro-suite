...
function installMockQuery(state) {
    pool.query = async (sqlText, params = []) => {
        ...
        if (sql.includes("from public.user_roles where user_id = $1 and company_id = $2 and role in ('admin', 'superadmin')")) {
            ...
            const isAdmin = state.roles.some((r) => r.user_id === userId && r.company_id === companyId && (r.role === 'admin' || r.role === 'superadmin'));
        ...
        if (sql.startsWith('insert into public.pets')) {
            const [companyId, clientId, name, species, breed, birthDate] = params;
        ...
        if (sql.startsWith('insert into public.appointments')) {
            const [, clientId, petId, serviceId, scheduledAt, durationMinutes, status, vetName] = params;
