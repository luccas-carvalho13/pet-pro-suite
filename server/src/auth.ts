...
export async function register(req: Request, res: Response) {
  ...
  try {
    ...
    const { rows: companyRows } = await pool.query(
      `INSERT INTO public.companies (name, cnpj, phone, address, status)
    ...
    const { rows } = await pool.query(
      ...
      [emailNorm, JSON.stringify({ full_name: full_name?.trim() ?? '', phone: user_phone?.trim() ?? '' }), hash]
    );
    ...
    await pool.query(
      ...
      [user.id, fullNameVal, emailVal, company.id]
    );
